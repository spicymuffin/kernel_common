...
static void destroy_inode(struct inode *inode)
{
	...
	// even if the someone opens the file again which might cause a race
	// in the tag insert path, it will have another ptr value because this
	// inode is dead at this point
	// so the tag missing in the hashtable (its rcu GP finishing faster than
	// the inode's rcu GP) wont be a problem

#ifdef CONFIG_PAPP
	per_app_destroy_inode_instrument(inode);
#endif
