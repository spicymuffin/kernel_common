...
void per_app_manager_exit(void)
{
	...
	list_for_each_entry_safe(app, tmp, &global_app_manager.app_list,
				 app_list) {
...
struct per_app *per_app_create(struct task_struct *task)
{
	...
	// TEMP: skip the sshd process for testing
 ...
 * page sanity check
 */
void per_app_page_check_anon_rmap(struct page *page, struct vm_area_struct *vma,
...
/*
 * per_app_move_anon_rmap, but use vma instead of pte
 */
...
int per_app_add_page(struct page *page, struct per_app *app)
{
	...
	if (WARN_ON_ONCE(TestClearPageLRU(page))) {
		pr_warn("[perapp]: this per-app page was managed by lru.\n");
...
struct per_app *per_app_select_app(void)
{
	...
	list_for_each_entry_reverse(app, &global_app_manager.app_list,
				    app_list) {
...
unsigned long per_app_calculate_nr_to_scan(struct per_app *app)
{
	...
	if (app_state >= RECLAIM_STATE_OOM)
		return 0UL;
	pages_to_reclaim =
		(nr_pages >> reclaim_ratio[app_state]) - nr_reclaimed;
					  ...
					  unsigned long nr_reclaimed)
{
	...
	unsigned long app_reclaimed_pages =
		atomic_long_add_return(nr_reclaimed, &app->nr_reclaimed);
	while (app->reclaim_state < RECLAIM_STATE_OOM &&
	       app_reclaimed_pages >= (per_app_get_page_count(app) >>
				       reclaim_ratio[app->reclaim_state])) {
		app->reclaim_state++;
	}
}
...
void per_app_print_reclaim_status(struct per_app *app)
{
	...
	pr_info("  [per_app_debug] UID: %u, State: %d, Pages: %lu (anon: %lu, file: %lu), Reclaimed: %lu\n",
		from_kuid(&init_user_ns, app->uid), app->reclaim_state,
 ...
 * =======================================
 */
static ssize_t per_app_reclaim_write(struct file *file,
...
static int __init per_app_create_proc_reclaim_file(void)
{
	...
	entry = proc_create("per_app_reclaim", 0222, NULL,
			    &per_app_reclaim_proc_ops);
	if (!entry) {
		pr_err("[perapp]: failed to create proc entry\n");
		return -ENOMEM;
	}
